#!/bin/bash
# L2-IG2 Monitor - Installateur Linux (Fedora / RHEL / CentOS / Rocky / Alma)
# Usage : chmod +x install-linux-fedora.sh && ./install-linux-fedora.sh
...
echo "======================================================"
echo "  L2-IG2 Monitor - Installateur Fedora / RHEL / CentOS"
echo "======================================================"
echo ""
...
if ! command -v python3 &> /dev/null; then
    ...
    if [ -n "$PKG_MGR" ]; then
        ...
        echo "❌ Gestionnaire de paquets non trouvé (dnf/yum requis)."
        exit 1
    fi
    if ! command -v python3 &> /dev/null; then
...
pip3 install psutil --quiet 2>&1 || \
    ...
    { [ -n "$PKG_MGR" ] && sudo $PKG_MGR install -y python3-psutil -q 2>&1; }
if ! python3 -c "import psutil" &>/dev/null; then
    echo "❌ Impossible d'installer psutil."
    echo "   Essayez : sudo $PKG_MGR install python3-psutil"
    exit 1
fi
...
if command -v curl &>/dev/null; then
    ...
    [ -n "$PKG_MGR" ] && sudo $PKG_MGR install -y curl -q
    curl -fsSL "${SERVER_URL}/api/download/agent" -o "$INSTALL_DIR/agent.py"
fi
